...
function Header()
{
	...
	$this->SetXY(135,18);
	$this->MultiCell(50,4,urldecode($ragsoc1),0,'L');
	$height = $this->getY();
	$this->SetXY(135,$height+2);
	$this->SetFont('Helvetica','',10);	
	$this->MultiCell(50,4,urldecode($indirizzo),0,'L');
	$height = $this->getY();
	$this->SetXY(135,$height);
	$this->MultiCell(50,4,urldecode($cap.' '.$localita.' '.'('.$provincia.')'),0,'L');
	$height = $this->getY();
	$this->SetXY(135,$height+2);
	$this->MultiCell(50,4,urldecode('Cort. Att. '),0,'L');
	...
	$this->MultiCell(50,4,urldecode($titolo . $titoloSpazio . $nome . ' ' . $cognome ),0,'L');
...
function Footer()
{
	//Footer string
		...
		$p->SetXY(10,$length + 15);

		$note2 = em(mb_convert_encoding(em($note), 'HTML-ENTITIES', 'UTF-8'));

		$p->MultiCell( 190, 4, urldecode($note2) . "\n\n" . "Restiamo in attesa di vs. cortese riscontro e con l'occasione porgiamo cordiali saluti." ,0,'L',1);


		$height = $p->getY();
		//Footer string
		$p->SetFont('Helvetica','',9);
		$p->SetXY(10,$height + 10);
		$p->MultiCell( 190, 4, $p->Image("./img/firma.jpg", null,null,40),0,'L',1);
