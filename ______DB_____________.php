...
connect($N,$V,$G){$this->_link=@mysqli_connect(($N!=""?$N:ini_get("mysql.default_host")),("$N$V"!=""?$V:ini_get("mysql.default_user")),("$N$V$G"!=""?$G:ini_get("mysql.default_password")),true,131072);if($this->_link)$this->server_info=mysqli_get_server_info($this->_link);else$this->error=mysqli_error($link);return(bool)$this->_link;}function
set_charset($Za){if(function_exists('mysqli_set_charset')){if(mysqli_set_charset($Za,$this->_link))return
true;mysqli_set_charset('utf8',$this->_link);}return$this->query("SET NAMES $Za");}function
quote($Q){return"'".mysqli_real_escape_string($link,$Q,$this->_link)."'";}function
select_db($k){return
mysqli_select_db($k,$this->_link);}function
query($H,$Ph=false){$I=@($Ph?mysqli_unbuffered_query($H,$this->_link):mysqli_query($link,$H,$this->_link));$this->error="";if(!$I){$this->errno=mysqli_errno($this->_link);$this->error=mysqli_error($this->_link);return
false;}if($I===true){$this->affected_rows=mysqli_affected_rows($this->_link);$this->info=mysqli_info($this->_link);return
true;}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
...
result($H,$o=0){$I=$this->query($H);if(!$I||!$I->num_rows)return
false;return
mysqli_result($I->_result,0,$o);}}class
Min_Result{var$num_rows,$_result,$_offset=0;function
__construct($I){$this->_result=$I;$this->num_rows=mysqli_num_rows($I);}function
fetch_assoc(){return
mysqli_fetch_assoc($this->_result);}function
fetch_row(){return
mysqli_fetch_row($this->_result);}function
fetch_field(){$J=mysqli_fetch_field($this->_result,$this->_offset++);$J->orgtable=$J->table;$J->orgname=$J->name;$J->charsetnr=($J->blob?63:0);return$J;}function
__destruct(){mysqli_free_result($this->_result);}}}elseif(extension_loaded("pdo_mysql")){class
Min_DB
extends
